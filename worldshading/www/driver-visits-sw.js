var CACHE_NAME = "ws-driver-visits-v2";
var APP_SHELL = [
    "/driver-visits",
    "/driver-visits-manifest.json",
    "/ws-pwa-core.css?v=1",
    "/ws-pwa-core.js?v=1"
];
    ...
    event.respondWith(
        fetch(event.request).then(function (response) {
            ...
            return caches.match(event.request).then(function (cached) {
                return cached || caches.match("/driver-visits");
            });
        })
    );
});
...
self.addEventListener("push", function (event) {
    ...
    event.waitUntil(
        self.registration.showNotification(data.title || "World Shading", {
            ...
            tag: data.tag || "worldshading",
            data: {
                url: data.url || "/driver-visits"
            }
        })
    );
});
    ...
    event.waitUntil(
        clients.matchAll({
            ...
            for (var i = 0; i < client_list.length; i++) {
                ...
                if (client.url.indexOf("/driver-visits") !== -1 && "focus" in client) {
                    return client.focus();
                }
            }
